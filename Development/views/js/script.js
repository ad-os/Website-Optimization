...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    ...
    var randomSize = document.querySelectorAll(".randomPizzaContainer")
    for (var i = 0; i < randomSize.length; i++) {
      randomSize[i].style.width = newWidth + "%";
    }
  }
...
function updatePositions() {
  ...
  var phaseArray = [];
  var leftPositionArray = [0, 256, 512, 768, 1024, 1280, 1536, 1792];
  for (var i = 0; i < 5; i++) {
    phaseArray.push(Math.sin((scrollTop / 1250) + (i)));
  }
  for (var i = 0; i < 24; i++) {
    var phase = phaseArray[i % 5];
    items[i].style.left = leftPositionArray[i % 8] + 100 * phase + 'px';
  }
